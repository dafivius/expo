...
function getTypedRoutesDeclarationFile(ctx) {
    ...
    const partialRouteTypes = expo_constants_1.default.expoConfig?.extra?.router?.partialRouteTypes || true;
    ...
    for (const type of groupedNodes.static) {
        staticRoutesStrings.push(contextKeyToType(type + "${`?${string}` | `#${string}` | ''}", partialRouteTypes));
        staticRoutesObjects.push(`{ pathname: ${contextKeyToType(type, partialRouteTypes)}; params?: UnknownInputParams | never; }`);
    }
    const staticRoutes = [
        '`./${string}`',
        '`../${string}`',
    ...
    for (const [dynamicRouteTemplate, paramsNames] of groupedNodes.dynamic) {
        ...
        dynamicRouteTypes.push(contextKeyToType(dynamicRouteTemplate
            ...
            .replaceAll(SLUG, '${Router.SingleRoutePart<T>}'), partialRouteTypes));
        dynamicRouteTemplateTypes.push(`{ pathname: ${contextKeyToType(dynamicRouteTemplate, partialRouteTypes)}, params: Router.UnknownInputParams${paramsString} }`);
    }
...
function contextKeyToType(contextKey, partialRoutes) {
